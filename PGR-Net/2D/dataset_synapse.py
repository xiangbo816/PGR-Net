...
class Synapse_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            label_path = os.path.join(self.data_dir[:-7]+'labels', slice_name+'.npy')
            label = np.load(label_path)
            image,label = augment_seg(self.img_aug, image, label)

            # train_transform = A.Compose([
            #     A.ElasticTransform(p=0.5, alpha=120, sigma=120 * 0.05, alpha_affine=120 * 0.03),
            #     A.CLAHE(p=0.3),  # 增强局部对比度，imgaug无
            #     A.ColorJitter(brightness=0.2, contrast=0, saturation=0.2, hue=0.1, p=0.3),  # 色调/饱和度调整，imgaug无
            #     A.GridDistortion(distort_limit=0.1, p=0.3),  # 网格畸变，与分段仿射互补
            # ])
            ...
            # image, label = data['image'][:], data['label'][:]
            data_path = os.path.join(self.data_dir, vol_name+'.npy')
            label_path = os.path.join(self.data_dir[:-7]+'labels', vol_name+'.npy')
            image = np.load(data_path)
            label = np.load(label_path)
