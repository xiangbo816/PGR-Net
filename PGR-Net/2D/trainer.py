...
def inference(model, testloader, args, test_save_path=None, device=None):
    model.eval()
    metric_list = 0.0
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        metric_i = test_single_volume(image, label, model, classes=args.num_classes, patch_size=[args.img_size, args.img_size],
                                      test_save_path=test_save_path, case=case_name, z_spacing=args.z_spacing,num_gpu = args.num_gpu,device=device)
...
def trainer_synapse(args, model, snapshot_path,device):
    ...
    logging.basicConfig(filename=snapshot_path + "/log_{}.txt".format(args.model_name), level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.info(str(args.operation))
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args.model_name))
    ...
    writer = SummaryWriter(args.log_dir + '/log_new/{}'.format(args.model_name))
    iter_num = 0
    ...
    for epoch_num in iterator:
        current_epoch_ce = 0.0
        current_epoch_dice = 0.0
        current_epoch_total = 0.0
        batch_count = 0  # 记录当前epoch内的批次数
        for i_batch, sampled_batch in enumerate(trainloader):
            image_batch, label_batch = sampled_batch['image'], sampled_batch['label']
            # print("data shape---------", image_batch.shape, label_batch.shape)
            image_batch, label_batch = image_batch.to(device, non_blocking=True), label_batch.squeeze(1).to(device, non_blocking=True)
            ...
            current_epoch_ce += loss_ce.item()
            current_epoch_dice += loss_dice.item()
            current_epoch_total += loss.item()
            batch_count += 1
        ...
        if batch_count > 0:
            # 计算当前epoch的平均损失
            avg_ce = current_epoch_ce / batch_count
            avg_dice = current_epoch_dice / batch_count
            avg_total = current_epoch_total / batch_count
            
            # 写入TensorBoard（以epoch为索引）
            writer.add_scalar('epoch/loss_ce', avg_ce, epoch_num + 1)  # epoch从1开始计数
            writer.add_scalar('epoch/loss_dice', avg_dice, epoch_num + 1)
            writer.add_scalar('epoch/total_loss', avg_total, epoch_num + 1)
            
            # 打印epoch级别的损失日志
            logging.info('epoch %d : avg_loss : %f, avg_ce: %f, avg_dice: %f' % 
                        (epoch_num + 1, avg_total, avg_ce, avg_dice))
        ...
        # if epoch_num + 1 >= 100 and (epoch_num + 1) % eval_interval == 0:
            filename = f'{args.model_name}_epoch_{epoch_num}.pth'
            save_mode_path = os.path.join(args.model_dir, filename)
            ...
            mean_dice, mean_hd95 = inference(model, testloader, args, test_save_path=test_save_path,device=device)
